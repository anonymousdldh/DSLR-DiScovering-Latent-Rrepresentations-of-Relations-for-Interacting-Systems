...
dim = n_f
origin_n_f = n_f + 2 # (random_value, mass) in our paper, we didn't use mass value.
...
def main():
    ...
    for i in range(len(data_test)):
        ...
        length_of_tails = length_of_tails, video_step = 1, axis0 = axis0, axis1 = axis1, xylim = xylim, video_save = args.video_save)
    ...
    if nr < 5 and not args.cmu:
        ...
        for n_r in range(nr):
            ...
            for idx in idx_visited:
                label[idx] = 0
            right_pred += label.max()
            idx_visited.append(label.argmax())
