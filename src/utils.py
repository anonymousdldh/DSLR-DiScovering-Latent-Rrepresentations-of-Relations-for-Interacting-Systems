...
def relation_fig_save(fig, train_name, data_name, suffix = ''):
    #fig.xlim(-0.002, 0.006)
    #fig.ylim(-0.0006, 0.0013)
    path = './results'
    if not os.path.exists(path):
        os.mkdir(path)
    path = './results/%s'%(train_name)
...
def video_generate_2d(train_name, data_name, i, ogn, rogn, x, e, steps, args, data_params, step_size = 1., cmu = False, length_of_tails = 75, video_step = 1, axis0 = 0, axis1 = 1, xylim = 10, relation_axis0 = 0, relation_axis1 = 1, video_save = True):
    t_max_see = args.t_max_see
